...
function Subscription_FruitSection({ addToCart, clearCart }) {
 ...
 // Retrieve the subscription ID from local storage or state
            ...
            const response = await axios.post('http://localhost:3001/subscription_products', {
            ...
            if (response.status === 200) {
                toast.success(`${product.productName} added to subscription in the database`);
            }
        } catch (error) {
            console.error('Error adding product to subscription in the database:', error);
